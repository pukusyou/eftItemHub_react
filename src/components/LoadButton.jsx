        ...
        bookMarkList.forEach(bookMark => {
            ...
            tags.push(
                <div
                    ...
                    className={`flex cursor-pointer items-center gap-3 rounded-lg border px-4 py-3 mb-2 transition-all duration-200 ${isSelected
                            ? 'border-amber-500/30 bg-amber-500/10'
                            : 'border-white/10 bg-bg-tertiary hover:border-white/20'
                        }`}
                >
                    <span className={`flex h-[18px] w-[18px] items-center justify-center rounded-full border-2 transition-all duration-200 ${isSelected
                    ...
                    <span className={`font-heading text-[0.95rem] font-semibold ${isSelected ? 'text-amber-500' : 'text-slate-100'
                        }`}>
...
const LoadButton = ({ setShowLoadModal, showLoadModal, bookMarkList, setIdList, setMissions }) => {
    ...
    const modalOpen = typeof showLoadModal === "boolean" ? showLoadModal : internalShow;
    ...
    const handleRemove = () => {
        if (selectedBookmark) {
            ...
            let json = JSON.stringify(getBookMarkList().filter(n => n !== selectedBookmark), undefined, 1);
