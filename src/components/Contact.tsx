  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setIsSending(false);
        setErrorMessage(
          data.message || "送信に失敗しました。入力ミスがないかご確認ください",
        );
      }
    } catch (error) {
  ...
  return (
    <>
      ...
      <button
        ...
        onMouseEnter={(e) => {
          e.currentTarget.style.transform = showModal
          ...
          >
            <form id="form" onSubmit={onSubmit}>
              ...
              <button
                type="submit"
                disabled={isSending}
                className={`mt-2 flex w-full items-center justify-center gap-2 rounded-md px-4 py-2 font-['Rajdhani'] text-sm font-bold tracking-[0.05em] text-black transition ${
                  isSending
