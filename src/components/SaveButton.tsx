...
function saveBookMark(name: string) {
  if (window.localStorage) {
    ...
    if (localStorage.getItem(bookMarkName) !== null) {
      const json = localStorage.getItem(bookMarkName);
    ...
    const json = JSON.stringify(array.concat(name), undefined, 1);
  ...
  const handleSave = () => {
    try {
      ...
      localStorage.setItem(
        "bookMark_" + textBox,
        JSON.stringify(taskIds, undefined, 1),
      );
    } catch (error) {
      setOk("");
