...
const Introduction = () => {
  return (
    <div className="mx-auto flex max-w-200 flex-col overflow-hidden rounded-lg border border-border bg-gradient-card transition-all duration-250 hover:border-accent-primary/30 hover:shadow-lg">
      <div className="flex flex-wrap flex-row">
        {/* Profile Image */}
        <div className="flex min-w-50 flex-[1_1_200px] items-center justify-center border-r border-border bg-bg-tertiary p-4">
          <img
            src={puku}
            alt="pukusyou"
            className="h-auto max-h-50 max-w-full object-contain rounded-md"
          />
        </div>
          ...
          <div className="grid gap-6 grid-cols-[repeat(auto-fit,minmax(200px,1fr))]">
            ...
            <div>
              ...
              <ul className="m-0 flex list-none flex-wrap gap-2 p-0">
                {["EscapeFromTarkov", "DQX", "Minecraft", "Valorant"].map(
                  (game, index) => (
                    <li
                      key={index}
                      className="rounded border border-border bg-bg-tertiary px-3 py-[0.35rem] text-[0.85rem] text-text-secondary"
                    >
                      {game}
                    </li>
                  ),
